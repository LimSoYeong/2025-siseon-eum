...
result_path = base_dir / "qwen" / "one_results_selected_prompts.jsonl"
image_path = base_dir / "data" / "img" / "img_041.jpg"  # 단일 파일
...
if doc_type == "고지서":
    ...
    print(f"⚠️ 알 수 없는 유형: {doc_type} (기본 고지서 프롬프트 사용)")
    prompt_text = A_PROMPT
...
print(f"📝 완료 ({infer_time:.2f}s)\n→ {output.strip()}")

# 저장 (원하면 아래 줄을 주석 처리해서 저장 끌 수 있음)
